...
class TableOperations:
    ...
    def create(table_name: str, field_info: List[Tuple[str, str]],
        ...
        if not os.path.exists(f'{cur_db_path}/{table_name}/table_index/delete_table.dbi'):
            joblib.dump({}, f'{cur_db_path}/{table_name}/table_index/delete_table.dbi')
    @staticmethod
    @decorator
    def delete(table_name: str,cur_db_path: str, field_info: List, cur_tables: dict) -> None:
        # 加载删除表
        delete_table = joblib.load(f'{cur_db_path}/{table_name}/table_index/delete_table.dbi')
        ...
        for table_index in table_indexs:
            ...
            while(row < block.shape[0]):
                ...
                if block.iloc[row,0] == field_info[0] and block.iloc[row,3] == 1:
                    ...
                    joblib.dump(delete_table, f'{cur_db_path}/{table_name}/table_index/delete_table.dbi')
                    # 保存
